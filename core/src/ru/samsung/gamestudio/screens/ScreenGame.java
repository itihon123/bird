...
public class ScreenGame implements Screen {
    ...
    public void show() {
        ...
        bird.hasShield = false; // Сброс щита
        initTubes();
    }
    ...
    public void render(float delta) {
        ...
        for (Tube tube : tubes) {
            ...
            if (tube.checkCoinCollision(bird)) {
                bird.hasShield = true;
            }

            if (tube.isHit(bird)) {
                if (bird.hasShield) {
                    tube.isDestroyed = true;
    ...
    public void dispose() {
        ...
        for (int i = 0; i < tubeCount; i++) tubes[i].dispose();
    }

    void initTubes() {
        tubes = new Tube[tubeCount];
        for (int i = 0; i < tubeCount; i++) tubes[i] = new Tube(tubeCount, i);
    }
}
